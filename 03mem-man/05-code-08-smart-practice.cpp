...
void f2(std::shared_ptr<std::string> shared_ptr)
{
    ...
    if (count == 2) std::cout << *shared_ptr;
...
void f3(std::weak_ptr<std::string> weak_ptr)
{
    ...
    // SOLUTION 5:
    // initially i tried to do weak_ptr = weak_ptr.lock(); // this didn't work because weak_ptr is of type std::weak_ptr
    auto shared = weak_ptr.lock();
    if (! weak_ptr.expired()) std::cout << (*shared);
    else std::cout << "weak pointer is expired" << std::endl;
