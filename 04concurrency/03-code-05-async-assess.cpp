...
/*
  Observe:
    1. a vector of futures is initialized as vector<future<typename>>
    2. when is synchronous execution faster? why?
...
struct LaunchParameters{
  ...
  // int nThreads, int nLoops, launch launchType
...
void Execute(LaunchParameters &lp){
  ...
  cout << "nThreads = " << nThreads << ", nLoops = " << nLoops << " , " << l_type << endl;
...
int main(){
  LaunchParameters lp1(100, 1e7, launch::async);
  LaunchParameters lp2(100, 1e7, launch::deferred);
  LaunchParameters lp3(100, 10, launch::async);
  LaunchParameters lp4(100, 10, launch::deferred);
